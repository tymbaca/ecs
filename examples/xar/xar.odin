...
*/
Array :: struct($SHIFT: uint) where 0 < SHIFT, SHIFT <= MAX_SHIFT {
...
*/
init :: proc(x: ^$X/Array($SHIFT), stride: int, allocator := context.allocator) {
	x^ = {stride = stride, allocator = allocator}
}
...
*/
clear :: proc "contextless" (x: ^$X/Array($SHIFT)) {
	x.len = 0
}

// Returns the length of the exponential-array
@(require_results)
len :: proc "contextless" (x: $X/Array($SHIFT)) -> int {
	return x.len
}

// Returns the number of allocated elements
@(require_results)
cap :: proc "contextless" (x: $X/Array($SHIFT)) -> int {
	#reverse for c, i in x.chunks {
		if c != nil {
...
*/
// @(require_results)
// get :: proc(x: ^$X/Array($SHIFT), #any_int index: int, loc := #caller_location) -> (val: T) #no_bounds_check {
...
*/
@(require_results)
get_ptr :: proc(x: ^$X/Array($SHIFT), #any_int index: int, loc := #caller_location) -> (val: rawptr) #no_bounds_check {
	runtime.bounds_check_error_loc(loc, index, x.len)
	chunk_idx, elem_idx, _ := _meta_get(SHIFT, uint(index))
	return &x.chunks[chunk_idx][elem_idx]
}
...
// No bounds checking
@(require_results)
get_ptr_unsafe :: proc "contextless" (x: ^$X/Array($SHIFT), #any_int index: int) -> (val: rawptr) #no_bounds_check {
	chunk_idx, elem_idx, _ := _meta_get(SHIFT, uint(index))
	return &x.chunks[chunk_idx][elem_idx]
}
...
*/
set :: proc(x: ^$X/Array($SHIFT), #any_int index: int, value: T, loc := #caller_location) #no_bounds_check {
	runtime.bounds_check_error_loc(loc, index, x.len)
	chunk_idx, elem_idx, _ := _meta_get(SHIFT, uint(index))
...
*/
push_back_elem :: proc(x: ^$X/Array($SHIFT), value: T, loc := #caller_location) -> (n: int, err: mem.Allocator_Error) {
...
*/
push_back_elems :: proc(x: ^$X/Array($SHIFT), values: ..T, loc := #caller_location) -> (n: int, err: mem.Allocator_Error) {
	for value in values {
...
*/
@(require_results)
push_back_elem_and_get_ptr :: proc(x: ^$X/Array($SHIFT), value: T, loc := #caller_location) -> (ptr: ^T, err: mem.Allocator_Error) {
	if x.allocator.procedure == nil {
		// to minic `[dynamic]T` behaviour
		x.allocator = context.allocator
	}
...
pop :: proc(x: ^$X/Array($SHIFT), loc := #caller_location) -> (val: T) {
...
@(require_results)
pop_safe :: proc(x: ^$X/Array($SHIFT)) -> (val: T, ok: bool) {
	if x.len == 0 {
		return
	}
	index := uint(x.len-1)
...
*/
unordered_remove :: proc(x: ^$X/Array($SHIFT), #any_int index: int, loc := #caller_location) {
	runtime.bounds_check_error_loc(loc, index, x.len)
	n := x.len-1
	if index != n {
		end := get(x, n)
...
// 	30
// */
// iterator :: proc(xar: ^$X/Array($SHIFT)) -> Iterator(SHIFT) {
...
// */
// iterate_by_val :: proc(it: ^Iterator($SHIFT)) -> (val: T, ok: bool) {
